...
constexpr const char *kLayoutAff = "aff";
...
public:
  ...
  ~MultipartInputStream() override { CloseAll(); }
  ...
  aff4::AFF4Status ReadBuffer(char *data, size_t *length) override {
    ...
    *length = 0;
    ...
    while (requested > 0 && current_part_ < files_.size()) {
      ...
      size_t to_read =
          static_cast<size_t>(std::min<uint64_t>(requested, part_remaining));
      size_t n = fread(data + *length, 1, to_read, files_[current_part_]);
      if (n == 0) {
        if (ferror(files_[current_part_]))
          return aff4::GENERIC_ERROR;
        ++current_part_;
        current_part_offset_ = 0;
        continue;
  ...
  aff4::AFF4Status Seek(aff4::aff4_off_t offset, int whence) override {
    ...
    for (size_t i = 0; i < files_.size(); ++i) {
      ...
      if (absolute <= walked + part_size) {
        ...
        if (fseek(files_[i], static_cast<long>(current_part_offset_),
                  SEEK_SET) != 0) {
          return aff4::GENERIC_ERROR;
        }
        for (size_t j = i + 1; j < files_.size(); ++j) {
          if (fseek(files_[j], 0, SEEK_SET) != 0)
            return aff4::GENERIC_ERROR;
        }
  ...
  aff4::aff4_off_t Size() const override { return size; }
...
public:
  ...
  ~MultipartOutputStream() override { CloseAll(); }
  ...
  aff4::AFF4Status Write(const char *data, size_t length) override {
    ...
    while (consumed < length) {
      ...
      size_t to_write = static_cast<size_t>(std::min<uint64_t>(
          remaining_in_part, static_cast<uint64_t>(length - consumed)));
...
bool IsTerminalInteractive() { return isatty(STDIN_FILENO) == 1; }

bool ParseNumericPartExtension(const std::string &path, int *part_number,
                               int *width) {
...
bool ParseAffNumericPart(const std::string &path, int *part_number,
                         int *width, std::string *base_name) {
  std::filesystem::path p(path);
  std::string ext = p.extension().string();
  ...
  std::string digits = stem.substr(last_underscore + 1);
  if (digits.empty())
    return false;
  ...
  *part_number = std::stoi(digits);
  *width = static_cast<int>(digits.size());
  *base_name = stem.substr(0, last_underscore);
  return true;
}

std::vector<std::string> GlobAffNumericSegments(const std::string &input_path) {
  int part = 0;
  int width = 0;
  std::string base_name;
  if (!ParseAffNumericPart(input_path, &part, &width, &base_name)) {
    return {input_path};
  }
  ...
  while (true) {
    ...
    out << '_' << std::setw(width) << std::setfill('0') << current;
    std::filesystem::path candidate = dir / (base_name + out.str() + ext);
...
bool HasEwfSegmentExtension(const std::string &path) {
  ...
  if (ext.size() != 4 || ext[0] != '.')
    return false;
  char kind =
      static_cast<char>(std::tolower(static_cast<unsigned char>(ext[1])));
  if (kind != 'e')
    return false;
...
std::vector<std::string> GlobRawNumericSegments(const std::string &input_path) {
  ...
  while (true) {
    std::filesystem::path candidate =
        dir / (base + FormatPartExtension(current, width));
    if (!std::filesystem::exists(candidate)) {
      break;
    }
    result.push_back(candidate.string());
...
std::string FindCompanionTxt(const std::string &input_path) {
  ...
  std::filesystem::path full_txt =
      p.parent_path() / (p.filename().string() + ".txt");
  if (std::filesystem::exists(full_txt))
    return full_txt.string();

  std::filesystem::path full_info =
      p.parent_path() / (p.filename().string() + ".info");
  if (std::filesystem::exists(full_info))
    return full_info.string();

  // Check <stem>.txt and <stem>.info (e.g. Test3E01.txt)
  std::filesystem::path stem_txt =
      p.parent_path() / (p.stem().string() + ".txt");
  if (std::filesystem::exists(stem_txt))
    return stem_txt.string();

  std::filesystem::path stem_info =
      p.parent_path() / (p.stem().string() + ".info");
  if (std::filesystem::exists(stem_info))
    return stem_info.string();
  // For .afd directories, the .info might be named after the directory
  std::filesystem::path parent = p.parent_path();
  std::string lower_parent = parent.string();
  std::transform(lower_parent.begin(), lower_parent.end(), lower_parent.begin(),
                 ::tolower);
  if (lower_parent.length() >= 4 &&
      lower_parent.substr(lower_parent.length() - 4) == ".afd") {
    std::filesystem::path afd_txt =
        parent.parent_path() / (parent.stem().string() + ".txt");
    if (std::filesystem::exists(afd_txt))
      return afd_txt.string();
    std::filesystem::path afd_info =
        parent.parent_path() / (parent.stem().string() + ".info");
    if (std::filesystem::exists(afd_info))
      return afd_info.string();
  }
...
BuildMultipartOutputPaths(const std::string &output_path,
                          const std::vector<std::string> &original_basenames) {
  std::vector<std::string> paths;
  if (original_basenames.empty())
    return paths;
...
void PrintUsage(const char *argv0) {
  std::cerr << "Usage:\n";
  std::cerr << "  " << argv0
            << " --archive-img|-a <dir> [--all-segments|-S] <inputs...>\n";
  std::cerr << "  " << argv0
            << " --extract-img|-x <dir> <map_name> <output_location>\n";
  std::cerr << "  " << argv0 << " --info <dir>\n";
  std::cerr << "  " << argv0 << " --help|-h\n";
  std::cerr << "\nFlags:\n";
  std::cerr << "  --all-segments, -S    For multi-volume images,\n";
  std::cerr
      << "                        automatically load all segment files.\n";
}
...
bool ParseArgs(int argc, char **argv, CliOptions *options, std::string *error) {
  ...
  if (extract_mode) {
    if (options->inputs.size() != 2) {
      *error = "Archive extraction requires <map_name> and <output_location>.";
      return false;
    }
...
int RunIngest(const CliOptions &options) {
  ...
  if (archive_store.Initialize() != aff4::STATUS_OK) {
    std::cerr << "Failed to initialize archive directory: "
              << options.archive_dir << "\n";
    return 1;
  }

  std::unordered_set<std::string> processed_afd_dirs;
  for (const std::string &input_path : options.inputs) {
    std::string lower_path = input_path;
    std::transform(lower_path.begin(), lower_path.end(), lower_path.begin(),
                   ::tolower);
    bool is_ewf = (lower_path.length() >= 4 &&
                   lower_path.substr(lower_path.length() - 4) == ".e01");
    bool is_aff4 = HasAff4Extension(input_path);
    bool is_aff = (lower_path.length() >= 4 &&
                   (lower_path.substr(lower_path.length() - 4) == ".aff" ||
                    lower_path.substr(lower_path.length() - 4) == ".afd"));
    int raw_part_number = 0;
    int raw_part_width = 0;
    bool is_raw_numeric_part = ParseNumericPartExtension(
        input_path, &raw_part_number, &raw_part_width);
    ...
    imgformatlib::Aff4ContainerStream aff4_stream(&resolver, image_urn);
    imgformatlib::AffStream aff_stream(&resolver, image_urn);
    ...
    if (is_ewf) {
      std::cout
          << "Detected EWF file, routing through imgformatlib::EwfStream...\n";
      std::vector<std::string> paths = {input_path};

      std::vector<std::string> segments =
          imgformatlib::EwfStream::GlobSegments(input_path);
      if (segments.size() > 1) {
        if (options.auto_all_segments) {
          std::cout << "Found " << segments.size() << " segments for "
                    << input_path
          ...
          if (IsTerminalInteractive()) {
            std::cout << "\nFound " << segments.size() - 1
                      << " continuing image segments for " << input_path
                      << ":\n";
            ...
            std::cout << "Found additional EWF segments but running "
                         "non-interactive; loading only primary file: "
                      << input_path << "\n";
          }
        }
      }
      ...
      if (paths.size() > 1) {
        try {
          for (const auto &p : paths) {
            source_part_sizes.push_back(std::filesystem::file_size(p));
          }
        } catch (const std::exception &e) {
          std::cerr << "Warning: Could not read EWF segment sizes: " << e.what()
                    << "\n";
        }
      }
    } else if (is_aff) {
      std::string actual_open_path = input_path;
      std::vector<std::string> paths = {input_path};

      // Check if this file is inside an .afd directory
      std::filesystem::path path(input_path);
      bool is_in_afd = false;
      if (path.has_parent_path()) {
        std::filesystem::path parent = path.parent_path();
        std::string parent_str = parent.string();
        std::string lower_parent = parent_str;
        std::transform(lower_parent.begin(), lower_parent.end(),
                       lower_parent.begin(), ::tolower);
        if (lower_parent.length() >= 4 &&
            lower_parent.substr(lower_parent.length() - 4) == ".afd") {
          is_in_afd = true;
          if (options.auto_all_segments) {
            actual_open_path = parent_str;
          } else {
            std::cout << "Detected other parts in the .afd folder ("
                      << parent_str << "). Ingest altogether? [Y/n] ";
            std::string resp;
      ...
      if (is_in_afd) {
        if (actual_open_path != input_path ||
            (lower_path.length() >= 4 &&
             lower_path.substr(lower_path.length() - 4) == ".afd")) {
          if (processed_afd_dirs.count(actual_open_path)) {
            std::cout << "Skipping " << input_path
                      << ". Parent AFD directory was already processed.\n";
            continue;
          }
          processed_afd_dirs.insert(actual_open_path);
          std::cout << "Detected multi-part AFF container (.afd). Processing "
                       "entire directory: "
                    << actual_open_path << "\n";
        }
        paths = {actual_open_path};
      } else {
        std::cout << "Detected legacy AFF file, routing through "
                     "imgformatlib::AffStream...\n";
      }

      // Ensure the URN reflects the .afd container if we switched to it
      if (actual_open_path != input_path) {
        image_urn = BuildImageUrn(actual_open_path);
      }

      paths = {actual_open_path};
      if (aff_stream.OpenRead(paths) != aff4::STATUS_OK) {
        std::cerr << "Failed to open legacy AFF container: " << paths[0]
                  << "\n";
        return 1;
      }
      active_stream = &aff_stream;
      source_layout = kLayoutAff;
      source_parts = paths;
      source_part_sizes.clear();

      // Record the file size of the segments if it's an .afd
      if (actual_open_path.length() >= 4 &&
          actual_open_path.substr(actual_open_path.length() - 4) == ".afd") {
        try {
          std::vector<std::filesystem::path> segment_files;
          for (const auto &entry :
               std::filesystem::directory_iterator(actual_open_path)) {
          ...
          for (const auto &f : segment_files) {
            source_part_sizes.push_back(std::filesystem::file_size(f));
          }
        } catch (const std::exception &e) {
          std::cerr << "Warning: Could not read AFD segment sizes: " << e.what()
                    << "\n";
        }
      }
    } else if (is_aff4) {
      std::cout << "Detected AFF4 container, mounting to ingest uncompressed "
                   "stream and RDF metadata...\n";
      std::vector<std::string> paths = {input_path};
      if (aff4_stream.OpenRead(paths) != aff4::STATUS_OK) {
        std::cerr << "Failed to open AFF4 container: " << input_path << "\n";
        return 1;
      }
      active_stream = &aff4_stream;
      ...
      if (segments.size() > 1) {
        if (options.auto_all_segments) {
          std::cout << "Found " << segments.size() << " raw image parts for "
                    << input_path
                    << ". Loading all automatically (--all-segments):\n";
          for (const auto &seg : segments)
            std::cout << "  " << seg << "\n";
          ...
          std::cout << "\nFound " << segments.size() - 1
                    << " continuing raw image parts for " << input_path
                    << ":\n";
          for (size_t i = 1; i < segments.size(); ++i)
            std::cout << "  " << segments[i] << "\n";
          ...
          std::cout << "Found additional raw parts but running "
                       "non-interactive; loading only primary file: "
                    << input_path << "\n";
        }
      }
      ...
      if (paths.size() > 1) {
        if (raw_multipart_stream.Open(paths) != aff4::STATUS_OK) {
          std::cerr << "Failed to open multipart raw input starting at: "
                    << input_path << "\n";
          return 1;
        }
        source_part_sizes = GetFileSizes(paths);
        if (source_part_sizes.size() != paths.size()) {
          std::cerr << "Failed to stat one or more raw image parts for: "
                    << input_path << "\n";
          return 1;
        }
        ...
        if (aff4::NewFileBackedObject(&resolver, input_path, "read",
                                      file_stream) != aff4::STATUS_OK) {
    ...
    // IngestStream computes and stores SHA-256 in a single pass — no separate
    // pre-read needed.
    std::string computed_sha256;
    if (archive_store.IngestStream(active_stream, image_urn,
                                   &computed_sha256) != aff4::STATUS_OK) {
    ...
    if (archive_store.StoreRawMetadata("source_layout:" + map_name,
                                       source_layout) != aff4::STATUS_OK) {
      std::cerr << "Failed to store source layout metadata for " << map_name
                << "\n";
      return 1;
    }
    if (archive_store.StoreRawMetadata(
            "source_parts:" + map_name,
            JoinLines(BasenamesOnly(source_parts))) != aff4::STATUS_OK) {
    ...
    if (!source_part_sizes.empty()) {
      ...
      for (uint64_t sz : source_part_sizes)
        sizes.push_back(std::to_string(sz));
      if (archive_store.StoreRawMetadata("source_part_sizes:" + map_name,
                                         JoinLines(sizes)) != aff4::STATUS_OK) {
        std::cerr << "Failed to store source part sizes metadata for "
                  << map_name << "\n";
        return 1;
      }
    }

    // Store companion .txt metadata file (case details, examiner, etc.) if
    // present.
    std::string txt_path;
    if (is_ewf) {
    ...
    if (!txt_path.empty()) {
      ...
      if (archive_store.StoreRawMetadata("sidecar_txt:" + map_name,
                                         txt_contents) == aff4::STATUS_OK) {
        std::cout << "Stored sidecar info file: " << txt_path << "\n";
      }
      if (is_ewf) {
        // Preserve compatibility with older EWF-specific key.
        archive_store.StoreRawMetadata("ewf_info_txt:" + map_name,
                                       txt_contents);
      }
    }

    if (is_aff4) {
      std::string turtle_content;
      if (aff4_stream.GetTurtleMetadata(&turtle_content) == aff4::STATUS_OK &&
          !turtle_content.empty()) {
        std::cout << "Extracted " << turtle_content.size()
                  << " bytes of RDF metadata. Storing in RocksDB.\n";
        archive_store.StoreRawMetadata("turtle_metadata:" + map_name,
                                       turtle_content);
      }
    }
  }
...
int RunExtract(const CliOptions &options) {
  ...
  // Auto-assign filename if output_path is a directory
  std::filesystem::path out_fs(output_path);
  if (std::filesystem::is_directory(out_fs) || out_fs.string().back() == '/' ||
      out_fs.string().back() == '\\') {
    std::string base = options.map_name;
    const std::string suffix = ".map.lz4";
    if (base.size() > suffix.size() &&
        base.substr(base.size() - suffix.size()) == suffix) {
      base = base.substr(0, base.size() - suffix.size());
    }
    if (base.substr(0, 4) == "map_") {
      base = base.substr(4);
    }
    // Basic cleanup of sanitized prefixes
    const std::string prefix_to_remove = "aff4___archive__";
    if (base.substr(0, prefix_to_remove.size()) == prefix_to_remove) {
      base = base.substr(prefix_to_remove.size());
    }
    output_path = (out_fs / base).string();
    std::cout << "Auto-assigned output file path: " << output_path << "\n";
    out_fs = std::filesystem::path(output_path);
  }

  std::string source_layout;
  bool have_layout =
      (extractor.RetrieveRawMetadata("source_layout:" + options.map_name,
                                     &source_layout) == aff4::STATUS_OK);
  bool is_ewf = have_layout && source_layout == kLayoutEwf;
  bool is_aff4 = have_layout && source_layout == kLayoutAff4;
  bool is_aff = have_layout && source_layout == kLayoutAff;
  bool is_raw_multipart = have_layout && source_layout == kLayoutRawMultipart;

  uint64_t split_size = 0;
  std::string sizes_str;
  if (extractor.RetrieveRawMetadata("source_part_sizes:" + options.map_name,
                                    &sizes_str) == aff4::STATUS_OK &&
      !sizes_str.empty()) {
    std::istringstream iss(sizes_str);
    std::string first_size_str;
    if (std::getline(iss, first_size_str) && !first_size_str.empty()) {
      try {
        split_size = std::stoull(first_size_str);
      } catch (...) {
      }
    }
  }
  ...
  if (!have_layout) {
    ...
    std::transform(lower_path.begin(), lower_path.end(), lower_path.begin(),
                   ::tolower);
    is_ewf = (lower_path.length() >= 4 &&
              lower_path.substr(lower_path.length() - 4) == ".e01");
    is_aff4 = HasAff4Extension(output_path);
    is_aff = (lower_path.length() >= 4 &&
              (lower_path.substr(lower_path.length() - 4) == ".aff" ||
               lower_path.substr(lower_path.length() - 4) == ".afd"));
  }
  ...
  imgformatlib::Aff4ContainerStream aff4_stream(&resolver,
                                                aff4::URN("aff4://extract/"));
  imgformatlib::AffStream aff_stream(&resolver, aff4::URN("aff4://extract/"));
  MultipartOutputStream multipart_output_stream(
      &resolver, aff4::URN("aff4://extract/multipart"));
  ...
  if (is_ewf) {
    ...
    if (ewf_output_base != output_path) {
      std::cout
          << "Detected explicit EWF segment extension; using base output path: "
          << ewf_output_base << "\n";
    }

    // Get the logical size from the map so libewf can properly initialise the
    // write handle.
    uint64_t logical_size = 0;
    if (extractor.GetMapLogicalSize(options.map_name, &logical_size) !=
        aff4::STATUS_OK) {
      std::cerr << "Warning: could not read logical size from map — writing "
                   "without media_size hint.\n";
    }

    if (ewf_stream.OpenWrite(ewf_output_base, logical_size, split_size) !=
        aff4::STATUS_OK) {
    ...
    if (extractor.GetMapLogicalSize(options.map_name, &logical_size) !=
        aff4::STATUS_OK) {
      std::cerr << "Warning: could not read logical size from map.\n";
    }

    if (aff_stream.OpenWrite(output_path, logical_size, split_size) !=
        aff4::STATUS_OK) {
      std::cerr << "Failed to open legacy AFF for writing: " << output_path
                << "\n";
      return 1;
    }
    active_stream = &aff_stream;
    ...
    if (aff4_stream.OpenWrite(output_path, logical_size) != aff4::STATUS_OK) {
      std::cerr << "Failed to open AFF4 output container: " << output_path
                << "\n";
      return 1;
    }

    std::string turtle_content;
    if (extractor.RetrieveRawMetadata("turtle_metadata:" + options.map_name,
                                      &turtle_content) == aff4::STATUS_OK &&
        !turtle_content.empty()) {
      aff4_stream.SetTurtleMetadata(turtle_content);
      std::cout << "Restored " << turtle_content.size()
                << " bytes of RDF metadata to container.\n";
    }
    ...
    if (extractor.RetrieveRawMetadata("source_parts:" + options.map_name,
                                      &parts_blob) != aff4::STATUS_OK ||
        extractor.RetrieveRawMetadata("source_part_sizes:" + options.map_name,
                                      &sizes_blob) != aff4::STATUS_OK) {
      std::cerr << "Failed to retrieve multipart metadata for map: "
                << options.map_name << "\n";
      return 1;
    }
    ...
    if (part_names.empty() || part_names.size() != part_sizes.size()) {
      std::cerr << "Invalid multipart metadata stored for map: "
                << options.map_name << "\n";
      return 1;
    }
    ...
    if (restored_part_paths.size() != part_sizes.size()) {
      std::cerr
          << "Failed to construct multipart output paths for extraction\n";
      return 1;
    }

    if (multipart_output_stream.Open(restored_part_paths, part_sizes) !=
        aff4::STATUS_OK) {
    ...
    if (aff4::NewFileBackedObject(&resolver, output_path, "truncate",
                                  file_stream) != aff4::STATUS_OK) {
  ...
  if (extractor.ExtractMap(options.map_name, active_stream) !=
      aff4::STATUS_OK) {
  ...
  bool have_sidecar =
      extractor.RetrieveRawMetadata("sidecar_txt:" + options.map_name,
                                    &txt_contents) == aff4::STATUS_OK;
  if (!have_sidecar && is_ewf) {
    have_sidecar =
        extractor.RetrieveRawMetadata("ewf_info_txt:" + options.map_name,
                                      &txt_contents) == aff4::STATUS_OK;
  }
  if (have_sidecar && !txt_contents.empty()) {
    std::filesystem::path txt_out;
    if (!restored_part_paths.empty()) {
      txt_out = std::filesystem::path(restored_part_paths.front())
...
void ReportArchiveMap(const std::filesystem::path &map_path) {
  ...
  map_file.read(reinterpret_cast<char *>(&uncompressed_size),
                sizeof(uncompressed_size));
  if (!map_file || uncompressed_size < sizeof(MapStreamHeader) +
                                           sizeof(SegmentHeader) +
                                           sizeof(MapStreamFooter)) {
    std::cout << "  - Map: " << map_path.filename().string()
              << " (invalid map header)\n";
    return;
  }
  ...
  const std::streamsize compressed_size =
      map_file.tellg() - static_cast<std::streamsize>(sizeof(uint64_t));
  ...
  const auto *header =
      reinterpret_cast<const MapStreamHeader *>(uncompressed.data());
  const auto *segment = reinterpret_cast<const SegmentHeader *>(
      uncompressed.data() + sizeof(MapStreamHeader));
  const auto *footer = reinterpret_cast<const MapStreamFooter *>(
      uncompressed.data() + uncompressed_size - sizeof(MapStreamFooter));
  const auto *refs = reinterpret_cast<const ChunkRef *>(
      uncompressed.data() + sizeof(MapStreamHeader) + sizeof(SegmentHeader));
  ...
  std::cout << "  - Disk Image: " << map_path.filename().string() << "\n"
            << "    Logical Extracted Size : " << header->logical_size
            << " bytes (" << (header->logical_size / 1024 / 1024) << " MB)\n"
            << "    Assigned Chunks        : " << segment->chunk_count << "\n"
            << "    Header Logical Size     : " << header->logical_size
            << " bytes\n"
            << "    Footer Logical Size     : " << footer->logical_size
            << " bytes\n"
            << "    Chunk Size Sum          : " << logical_sum << " bytes\n"
            << "    Footer Chunk Count      : " << footer->chunk_count << "\n";

  if (header->logical_size != footer->logical_size ||
      logical_sum != header->logical_size ||
      segment->chunk_count != footer->chunk_count) {
...
int RunInfo(const CliOptions &options) {
  ...
  column_families.push_back(rocksdb::ColumnFamilyDescriptor(
      "chunk_index_cf", rocksdb::ColumnFamilyOptions()));
  column_families.push_back(rocksdb::ColumnFamilyDescriptor(
      "metadata_cf", rocksdb::ColumnFamilyOptions()));

  std::vector<rocksdb::ColumnFamilyHandle *> handles;
  rocksdb::DB *db = nullptr;
  uint64_t unique_chunks = 0;
  rocksdb::Status status = rocksdb::DB::OpenForReadOnly(
      db_options, (archive_dir / "chunk_corpus").string(), column_families,
      &handles, &db);
  if (status.ok()) {
  ...
  std::cout << "Total Physical Bevy Disk Size    : " << physical_size
            << " bytes (" << (physical_size / 1024 / 1024) << " MB)\n\n";
